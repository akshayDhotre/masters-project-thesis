...
void main(int argc, char *argv[])
{
	...
	n=tf/1;
	// Change value in for loop 1st line also******* 
	...
	for (i=1;i<=n;i++) 
  	{  
		...
		dutycycle_per[i]=(dutycycle[i]/100);		
		
		printf("\n %f \n %d",dutycycle[i],dutycycle_per[i]);
	...
	softPwmWrite(0,phi_per[i]);
	
	softPwmWrite(1,dutycycle_per[i]);
